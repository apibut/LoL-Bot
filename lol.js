...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            + 'VERSION:3.0\n'  // Jangan di ubah biar ga error
            + 'FN:Mr.1541\n'  // Ganti jadi namamu
            + 'ORG: Pengembang Mr.1541;\n'  // Ganti jadi namamu/Botmu
            + 'TEL;type=CELL;type=VOICE;waid=6281325714976:+6281325714976\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
async function starts() {
   ...
   if (cr == "") {
    ...
    lolteam.on('chat-update', async(lol) => {
        try {
            ...
            global.prefix
	    const ownerNumber = ["6281325714976@s.whatsapp.net"] // owner number ubah aja
 ...
 if (isCmd) cmdadd()
		...
		if (!isRegistered) return reply(ind.noregis())
                    var punya_wa = "0@s.whatsapp.net"
                    var ini_text = (cr)
                    var ini_buffer = await getBuffer("https://i.ibb.co/KLRM8Nf/IMG-20210329-WA1106.jpg")
...
lolteam.sendMessage(from, fs
readFileSync('./lib/1617835492428.jpg'), sticker, {quoted: mek})
}
		//DAFTAR\\
		case 'daftar':
		...
		if (!isRegistered) return reply(ind.noregis())
                    if ((isMedia && !lol.message.videoMessage || isQuotedSticker)) {
                        if (args.length == 0) return reply(`Example: ${prefix + command} isal|gans`)
                        const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(lol).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : lol
		...
		if (!isRegistered) return reply(ind.noregis())
                ...
                lolteam.sendMessage(from, 'Tuh kontak Mr.1541 <•_•>',MessageType.text, { quoted: lol} )
	        ini_buffer = await getBuffer("https://i.ibb.co/KLRM8Nf/IMG-20210329-WA1106.jpg")
                lolteam.sendMessage(from, ini_buffer, MessageType.image, {quoted: lol, caption: '*SUBSCRIBE:*\n*https://www.youtube.com/channel/UCyKYqyGq6Y-o4sIMmiYvV7g*'})
                 break
				case 'addprem':
				if (!isRegistered) return reply(ind.noregis())
					if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK cowner')
                ...
                case 'artinama':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} Adul`)
                    ini_nama = args.join(" ")
		...
		if (!isRegistered) return reply(ind.noregis())
                ...
                case 'faketoko':
		if (!isRegistered) return reply(ind.noregis())
                    await faketoko(teks = "Tahu Bacem", url_image = "https://i.ibb.co/JdfQ73m/photo-2021-02-05-10-13-39.jpg", title = "Adul", code = "IDR", price = 1000000)
                    break
    ...
    case 'ttp':
		...
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK cowner')
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nExample: ${prefix + command} Adul xnxx.com`)
		username = args[0]
		comment = args[2]
		...
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK cowner')
                    if (args.length == 0) return reply(`Usage: ${prefix + command} nik|provinsi|kabupaten|nama|tempat, tanggal lahir|jenis kelamin|jalan|rt/rw|kelurahan|kecamatan|agama|status nikah|pekerjaan|warga negara|berlaku sampai|url_image\n\nExample: ${prefix + command} 456127893132123|bumipertiwi|fatamorgana|Adul|mars, 99-99-9999|belum ditemukan|jl wardoyo|999/999|turese|imtuni|alhamdulillah islam|jomblo kack|mikirin dia|indo ori no kw|hari kiamat|https://i.ibb.co/Xb2pZ88/test.jpg`)
